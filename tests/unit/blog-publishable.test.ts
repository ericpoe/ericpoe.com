...
import { isPublishable, postTerms } from '../../src/utils/blog';
import type { BlogPost } from '../../src/utils/blog';

const NOW = new Date('2026-09-10T00:00:00Z').getTime();

function post(overrides: { draft?: boolean; date?: string; categories?: string[]; tags?: string[] }): BlogPost {
  return {
    data: {
      draft: overrides.draft ?? false,
      date: new Date(overrides.date ?? '2026-01-01T00:00:00Z'),
...
describe('postTerms', () => {
  it('lists categories before tags', () => {
    expect(postTerms(post({ categories: ['tech', 'ai'], tags: ['llm'] }))).toEqual(['tech', 'ai', 'llm']);
  });

  it('tolerates a post with neither', () => {
    expect(postTerms(post({}))).toEqual([]);
  });
});
